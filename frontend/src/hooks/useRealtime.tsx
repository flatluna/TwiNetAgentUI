...
type Parameters = {
    ...
    onReceivedLocationConfirmation?: (message: any) => void;
    onReceivedAgentTextResponse?: (message: any) => void;
    onReceivedDestinationCoordinates?: (message: any) => void;
    onReceivedRouteInformation?: (message: any) => void;
...
export default function useRealTime({
    ...
    onReceivedLocationConfirmation,
    onReceivedAgentTextResponse,
    onReceivedDestinationCoordinates,
    onReceivedRouteInformation,
    onReceivedError
}: Parameters) {
    const wsEndpoint = useDirectAoaiApi
        ...
        : import.meta.env.DEV
          ? "ws://localhost:8000/voice" // Connect to our voice-enabled AutoGen agent
          : `/realtime`; // Proxy in production
    ...
    const sendUserLocation = (latitude: number, longitude: number) => {
        const locationMessage = {
            type: "user_location",
            data: { latitude, longitude }
        };

        console.log("[DEBUG] Sending user location via WebSocket:", locationMessage);
        sendJsonMessage(locationMessage);
    };
    ...
    const sendTextMessage = (text: string) => {
        ...
        console.log("[DEBUG] Sending user text via WebSocket:", textMessage);
        sendJsonMessage(textMessage);
    };
    ...
    const onMessageReceived = (event: MessageEvent<any>) => {
        ...
        console.log("[DEBUG] useRealtime - Message received:", message.type);
        if (message.type === "destination_coordinates") {
            console.log("[DEBUG] useRealtime - DESTINATION_COORDINATES message detected!");
            console.log("[DEBUG] useRealtime - Full message:", message);
        }
        ...
        switch (message.type) {
            ...
            case "location_confirmed":
                onReceivedLocationConfirmation?.(message);
                break;
            case "agent_text_response":
                onReceivedAgentTextResponse?.(message);
                break;
            case "destination_coordinates":
                console.log("[DEBUG] useRealtime - Processing destination_coordinates message");
                console.log("[DEBUG] useRealtime - Message:", message);
                onReceivedDestinationCoordinates?.(message);
                break;
            case "route_information":
                console.log("[DEBUG] useRealtime - Processing route_information message");
                console.log("[DEBUG] useRealtime - Message:", message);
                onReceivedRouteInformation?.(message);
                break;
            case "error":
    ...
    return { startSession, addUserAudio, inputAudioBufferClear, sendUserLocation, sendTextMessage };
}
